...
export function Catalog({ products }: { products: Product[] }) {
  ...
  const cards = (items: Product[]) => <div className="grid">{items.map((item) => <article className="card" key={item.id}>
    <button className="image-wrap product-image-button" onClick={() => setSelected(item)} aria-label={`Ver ${item.name}`}><Image src={item.image_url} alt={item.name} fill quality={95} sizes="(max-width: 640px) 55vw, (max-width: 1100px) 38vw, 30vw"/></button>
  ...
  return <>
    ...
    {selected && <div className="product-lightbox" role="dialog" aria-modal="true" onClick={() => setSelected(null)}><div className="lightbox-panel" onClick={(event) => event.stopPropagation()}><button className="lightbox-close" onClick={() => setSelected(null)} aria-label="Cerrar"><X/></button><div className="lightbox-image"><Image src={selected.image_url} alt={selected.name} fill sizes="95vw"/></div><div className="lightbox-caption"><strong>{selected.name}</strong><span>{formatGuarani(selected.price)}</span></div><a className="lightbox-share" href={whatsappUrl(selected.name, selected.reference, selected.price)} target="_blank" rel="noreferrer"><WhatsAppIcon size={18}/> Consultar</a></div></div>}
  </>;
}
